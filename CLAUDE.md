...
### GitHub auth (`src/github/`)

PAT-only today. `GITHUB_PAT` is required in `env.ts`; `github/client.ts` returns the PAT-backed Octokit. The `profiles.github_installation_id` column is reserved for a future GitHub App mode and is otherwise unused.
